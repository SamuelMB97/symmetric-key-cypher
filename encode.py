...
def loadkeys(master_key64):
    # I thought I had to remove every 8...
    """temp = [c for c in master_key64]
    print(f"{str_w_len(temp)} - temp")
    for idx in [64, 56, 48, 40, 32, 24, 16, 8]:
        temp.pop(idx-1)
    
    print(f"{str_w_len(temp)} - temp")

    key_used56 = "".join(temp)
    print(str_w_len(key_used56))
    """
    k_perm_i = permute(master_key64, db.pc_1)
    for j in [n+1 for n in range(16)]: # j = key round number
        subkey56 = k_perm_i
        yield subkey56
...
def main(message, key):
    ...
    key_bin = hex_to_bin(k)
    print(f"{str_w_len(key_bin)} - key_bin")
    subkey = loadkeys(key_bin)
